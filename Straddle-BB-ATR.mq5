...
input double TP_Points = 100;            // 100 pips take profit
input double Recovery_TP = 50;           // 50 pips for recovery trades
input int    Recovery_Step = 100;        // 100 pips between recoveries
...
{
   datetime current[];
   if(CopyTime(_Symbol,_Period,0,1,current) != 1) return false;
...
{
   if(ArraySize(buyTickets) == 0) return;
...
{
   if(ArraySize(sellTickets) == 0) return;
